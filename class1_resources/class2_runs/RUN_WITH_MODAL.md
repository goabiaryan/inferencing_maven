...
> **Note:** Replace any occurrences of your local path (e.g. `/Users/jarvis/...`) with
> `<PATH_TO_CLASS1_RESOURCES>` or set `export CLASS_ROOT="<PATH_TO_CLASS1_RESOURCES>"`
> and use `$CLASS_ROOT` in commands. Students should fill in their own paths.
...
Copy the printed Modal web URL (e.g. `<YOUR_MODAL_URL>`).
...
Use **three terminals**. Base path: `<PATH_TO_CLASS1_RESOURCES>`. Adjust paths if your repo is elsewhere.
